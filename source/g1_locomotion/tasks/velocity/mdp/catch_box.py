...
def active_box_state_b(
    ...
    return torch.cat((box_pos_b, box_quat_b, box_lin_vel_b, box_ang_vel_b, box_size), dim=-1)
...
def palm_box_kinematics_b(
    ...
    return torch.cat((position_delta_b.flatten(1), velocity_delta_b.flatten(1)), dim=-1)
...
def hand_target_proximity(
    ...
    targets, palms, _, _ = _palm_targets_w(env, collection_name, robot_cfg, clearance)
    mean_distance = torch.linalg.vector_norm(targets - palms, dim=-1).mean(dim=1)
    return torch.exp(-mean_distance / std)
...
def box_centered_between_palms(
    ...
    _, palms, _, box = _palm_targets_w(env, collection_name, robot_cfg)
    midpoint_error = torch.linalg.vector_norm(palms.mean(dim=1) - box[:, :3], dim=-1)
    return torch.exp(-midpoint_error / std)
...
def palm_box_velocity_match(
    ...
    _, _, palm_vel, box = _palm_targets_w(env, collection_name, robot_cfg)
    relative_speed = torch.linalg.vector_norm(palm_vel.mean(dim=1) - box[:, 7:10], dim=-1)
    return torch.exp(-relative_speed / std)
...
def bilateral_hand_contact(
    ...
    return torch.minimum(left_score, right_score)
...
def _catch_condition(
    ...
    targets, palms, palm_vel, box = _palm_targets_w(env, collection_name, robot_cfg)
...
def box_out_of_reach(
    ...
    box, _ = _active_box(env, collection_name)
    robot: Articulation = env.scene[robot_cfg.name]
    return torch.linalg.vector_norm(box[:, :3] - robot.data.root_pos_w, dim=-1) > max_distance
